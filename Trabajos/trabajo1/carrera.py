...
while puedes_seguir_jugando:
    ...
    seleccionados= sample(todas, 10) # selecciona 10 tortugas al azar de la lista de todas las tortugas para que corran en la carrera, y asi cada carrera sea diferente y mas entretenida

    for t in seleccionados:
        tortuga = Tortuga()
        tortuga.crear_tortuga(t["nombre"], t["frase_ganador"], t["frase_top3"], t["frase_perdedor"])
        tortuga.distancia_recorrida = 0
    ...
    #revisar si el usuario gano o perdio la apuesta, y sumar o restar el dinero dependiendo de eso
    ...
    elif elegida == posiciones[1].nombre:
        dinero_total += apuesta // 2
        print(Style.BRIGHT + Fore.YELLOW + f"¡Casi! {elegida.strip()} ha quedado en el segundo lugar. TTu nueva cantidad de dinero es: {dinero_total}\n" + Style.RESET_ALL)
    
    elif elegida == posiciones[2].nombre:
        print(Style.BRIGHT + Fore.YELLOW + f"¡Casi! {elegida.strip()} ha quedado en el tercer lugar. Tu cantidad de dinero se mantiene: {dinero_total}\n" + Style.RESET_ALL)
    else:
        dinero_total -= apuesta
        print(Style.BRIGHT + Fore.RED + f"Lo siento, {elegida.strip()} no ha ganado. Tu nueva cantidad de dinero es: {dinero_total}\n" + Style.RESET_ALL)


    #preguntar si quiere seguir jugando o no, y revisar si se quedo sin dinero para apostar
    if dinero_total <= 0:
        print(Style.BRIGHT + Fore.RED + "¡Oh no! Te has quedado sin dinero para apostar. ¡Gracias por jugar!" + Style.RESET_ALL)
        puedes_seguir_jugando = False
    else:
        seguir = input("¿Quieres jugar otra carrera? (si/no): ")
        if seguir.lower() != 'si':
            print(Style.BRIGHT + Fore.CYAN + "¡Gracias por jugar! ¡Hasta la próxima!" + Style.RESET_ALL)
            puedes_seguir_jugando = False